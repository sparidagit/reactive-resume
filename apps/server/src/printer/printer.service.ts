...
private readonly s3Client: S3Client;
  ...
  async generateResume(resume: ResumeDto) {
    ...
    const secretAccessKey = this.configService.get<string>("AWS_SECRET_ACCESS_KEY");
      ...
      const extension ="pdf";
    const filepath = `${resume.userId}.pdf`;
      try {
        const params = {
          
          Bucket: 'talentstream-resume',
          Key: filepath,
          Body: buffer,
        ...
        const url = `https://talentstream-resume.s3.ap-south-1.amazonaws.com/${filepath}`;
        await this.redis.set(`user:${resume.userId}:storage:pdf:test`, url);
        return url;
      } catch (error) {
